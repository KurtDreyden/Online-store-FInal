...
if (isset($_GET['id'])) {
    ...
    $shoes = $result->fetch_assoc(); //an associative array of a row returend from the database that can be used
    ...
    array_push($_SESSION['product'], $newshoes);
}
...
if (isset($_POST['add'])) {
    ...
    $item_name = $newshoes->getName();
    $item_price  = $newshoes->getPrice();
...
<head>
    ...
    <style>
        ...
        .image-wrapper {
            height: 450px;
            width: 300px;
            background-image: url("<?php echo $newshoes->getThumbnail() ?>");
...
<body>
    ...
    <div class="shop-container">
        ...
        <div class="description">
            <span>
                <p id="title"><?php echo  $newshoes->getName()?></p>
               
            </span>
            <span>
                <p><?php echo $newshoes->displayStock().'('.$newshoes->getQuantity().')'?></p>

                <?php if ($newshoes->getQuantity() != 0) : ?>
                    <p>Price: R <?php echo $newshoes->getPrice() ?></p>

                    <form action="" method="POST">
                        <input type="number" id="quantity" name="quantity" placeholder="quantity">
                        <button type="submit" id="add-to-cart" value='<?php echo $newshoes->getPrice() ?>' name="add">Add to Cart</button>
                    </form>

                    <form action="" method="POST">
                        <button type="submit" id="add-to-cart" value='<?php echo $newshoes->getPrice() ?>' name="delete">Remove from cart</button>
                    </form>
                    <form action="" method="POST">
                        <button type="submit" id="add-to-cart" value='<?php echo $newshoes->getPrice() ?>' name="clear">Clear Cart</button>
                    </form>
